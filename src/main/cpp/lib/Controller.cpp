...
Controller::Controller(int slot, std::unordered_map<std::string, std::shared_ptr<frc2::Command>> &actions) : controller{slot}, actionMap{actions} 
{
...
bool Controller::LoadControls()
{
    ...
    {
        for (auto& mode : controllerMap[controller.GetName()])
        {
            for (auto& action : mode.second)
            {
                action.second.WhileTrue(NULL);
